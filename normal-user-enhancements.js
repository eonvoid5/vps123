...
function css(){
 ...
 #void-normal-specs{display:block;margin-top:14px;padding:18px;border:1px solid rgba(70,250,155,.14);border-radius:16px;background:rgba(2,11,7,.48);box-shadow:0 0 28px rgba(40,245,141,.05)}
...
function findPanel(title){return [...document.querySelectorAll('.panel,.card')].find(p=>{const h=p.querySelector('.panel-title h2,.panel-title h3,h2,h3');return text(h)===title.toLowerCase()})}
function normalizeDashboard(){
 ...
 let box=document.getElementById('void-normal-specs');
 if(box && !document.body.contains(box)) box=null;
 if(!box){
